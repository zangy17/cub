import numpy as np
import json
mutual_info = np.load('mutual_info_xx2.npy')

ids = np.argsort(mutual_info)[::-1]

#print(1500,mutual_info_s[ids2[1500]])
#print(2000,mutual_info_s[ids2[2000]])

with open('all_att_xx2.json', 'r') as fp:
    concepts = json.load(fp)
class_label=np.load('class_label_des_xx2.npy')
...
with open('all_att_of_new.json','w') as fw:
    json.dump(new_concepts,fw)
np.save('class_label_des_of_new.npy',new_class_label)
...
with open('cub_of_new.json','w') as fw:
    json.dump(cub_fff,fw)
with open('full_cub_of_new.json','w') as fw:
