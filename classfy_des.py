...
data, _, _ = load_data(data_root, split="valid")
class_id_dict = {}
...
loader = DataLoader(test_set, batch_size=512, num_workers=8)
...
def zeroshot_classifier_gpt(textnames):
    '''
    class_label = np.load('class_label_des_new.npy')
    class_label = torch.Tensor(class_label).to(device)
    with open('all_att_new.json','r') as fp:
        all_att = json.load(fp)
    with torch.no_grad():
        texts = tokenizer(all_att).to(device)
        embeddings = model_text(texts)
        embeddings =F.normalize(embeddings,dim=-1)
        new_weights = class_label @ embeddings
        new_weights = F.normalize(new_weights,dim=-1)
    ...
    with torch.no_grad():
        ...
        zeroshot_weights = torch.stack(zeroshot_weights, dim=1).to(device)
    #print(list(new_weights[0].cpu().numpy()))
    #print(list(zeroshot_weights[0].cpu().numpy()))
...
model.load_state_dict(torch.load('checkpoint/cub_des_of99.pt', map_location='cuda:0'))
