...
x2 = [1,1,1,1,1,1,0,1,1]
y = [0.1,0.2,0.1,0.8,0.75,0.62,0.33,0.19,0.17]
print(adjusted_mutual_info_score(x2,y))
'''
...
with torch.no_grad():
    ...
    with open('all_att_f95.json','r') as fp:
        concepts=json.load(fp)
    class_label=np.load('class_label_des_f95.npy')
    mutual_info = []
    for i in tqdm(range(len(concepts))):
        x = []
        for j in range(len(labels)):
            if class_label[labels[j]][i]==1:
                x.append(1)
            else:
                x.append(0)
        ...
        y = t_embed@image_embeddings.T
        y = y.squeeze(0)
        y = torch.nn.functional.softmax(y,dim=0)
        y = y.cpu().numpy()
        mutual_info.append(adjusted_mutual_info_score(x, y))
print(mutual_info)
...
np.save('nutual_info_softmax.npy',mutual_info)
