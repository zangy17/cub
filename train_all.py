...
with open('pred_labelsxx2.json','r') as fj:
...
#fjirfgurhfh
optimizer = torch.optim.AdamW(filter(lambda p:p.requires_grad, model.parameters()), lr=5e-5, weight_decay=1e-5)
...
for epoch in range(100):
    ...
    if epoch %10==9:
        torch.save(model.state_dict(), 'checkpoint/cub_des_all' + str(epoch) + '.pt')
