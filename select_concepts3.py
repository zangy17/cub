...
concept2class ={}
itt=0
class_names = list(old_cub.keys())
for id,c in enumerate(old_cub):

    for con in old_cub[c]:
...
with open('all_att_x.json','w') as fw:
    json.dump(new_concepts,fw)
with open('cub_x.json','w') as fw:
...
for i in range(len(new_cub)):
    for j in range(len(new_concepts)):
        if new_concepts[j] in new_cub[class_names[i]]:
            class_label[i][j]=1
np.save('class_label_des_x.npy',class_label)
