...
def main():
    ...
    # LOAD TRAIN IMAGES
    train_data, train_y, _, classes = load_data('/home/user', split="train")

    # LOAD TEST IMAGES
    test_data, test_y, _, classes = load_data('/home/user', split="valid")
    cls_truth = np.load('class_label_des_xx2.npy')


    # primitive concept activations
    tr_att_activations = torch.Tensor(np.load('save_des' + "/attribute_activations_train_all.npy"))
    t_att_activations = torch.Tensor(np.load('save_des' + "/attribute_activations_valid_all.npy"))
