...
with open('pred_labels.json','r') as fj:
    train_labels = json.load(fj)
train_set = CubDataset(train_image_paths,train_labels,test_preprocess,tokenizer)
...
test_loader = DataLoader(dataset=test_set,batch_size=batch_size,num_workers=32)
with open('final_all_att.json','r') as fp:
    texts = json.load(fp)
...
cls_truth = np.load('class_label_des_final.npy')
...
for epoch in range(100):
    ...
    for (images, targets, num) in tqdm(train_loader):
        ...
        pred = torch.matmul(image_features, text_features.t()) * model.logit_scale.exp()
        ...
        for j in range(len(targets)):
            if pred_label[j] == targets[j]:
                correct += 1.
        total += len(images)

    scheduler.step()
    print('epoch:', epoch, 'epoch loss:', np.mean(avg_loss / total * batch_size),'train acc:',correct/total*100)
    if epoch %10==9:
        test_correct = 0
        test_total =0
        torch.save(model.state_dict(), 'checkpoint/cub_des_f' + str(epoch) + '.pt')
        model.eval()
        for (images, targets, num) in tqdm(test_loader):
            images = images.to(device)
            targets = targets.to(device)
        ...
        print('test acc:', test_correct/test_total*100)